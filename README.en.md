...
| 👥 **Groups / guest mode** | Mention or reply in allowed groups (even without being a member, via guest mode): **ephemeral** answer visible only to the asker (`#todos` = public). Groups never get approval prompts: anything outside the allowlist is denied. |
