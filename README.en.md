...
| 👥 **Groups / guest mode** | In allowed groups, triggered by a **reply** to a bot message, by **`/ask <question>`**, or by a mention (`@bot` only arrives with *Group Privacy* off — Telegram doesn't deliver mentions in privacy mode). Without being a member, via guest mode. **Ephemeral** answer visible only to the asker (`#todos` = public). Groups never get approval prompts: anything outside the allowlist is denied. |
